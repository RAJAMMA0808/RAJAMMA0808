...
import { College, DashboardData, Role, User } from '@shared/types';
...
const Dashboard: React.FC<DashboardProps> = ({ user }) => {
  ...
  const [filterConflictError, setFilterConflictError] = useState<string | null>(null);

  // --- Filter Exclusivity & Conflict Logic ---
  useEffect(() => {
    if (selectedFacultyId !== 'all' && selectedStaffId !== 'all') {
      setFilterConflictError("You’ve selected both Faculty ID and Staff ID. Please choose one filter at a time.");
    } else {
      setFilterConflictError(null);
    }
    ...
    if (selectedFacultyId !== 'all') {
      ...
      // setSelectedStaffId('all'); // Don't auto-clear to allow conflict check
    }
  }, [selectedFacultyId, selectedStaffId]);
  ...
  useEffect(() => {
      if (selectedStaffId !== 'all') {
          ...
          setSelectedYear('all');
          setSelectedRollNo('all');
          // setSelectedFacultyId('all'); // Don't auto-clear to allow conflict check
      }
  }, [selectedStaffId]);
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    setLoading(false);
  }, [selectedCollege, selectedYear, selectedDepartment, selectedRollNo, selectedFacultyId, selectedStaffId, selectedDate, filterConflictError]);
 ...
 const handleDownload = async () => {
    ...
    try {
        if (selectedFacultyId !== 'all') {
            ...
            const headers = ['Staff ID', 'Staff Name', 'College', 'Department', 'Gender', 'Total Days', 'Present Days', 'Absent Days', 'Half Days'];
            const csvRows = [headers.join(',')];
            staffData.forEach(s => {
                const row = [`"${s.staffId}"`, `"${s.staffName}"`, `"${s.collegeCode}"`, `"${s.programCode}"`, `"${s.gender}"`, s.totalDays, s.presentDays, s.absentDays, s.halfDays];
                csvRows.push(row.join(','));
            });
...
const downloadCsv = (rows: string[], filename: string) => {
  ...
  const commonSelectClass = "h-9 bg-slate-800 border border-slate-700 text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-2.5 disabled:bg-slate-700 disabled:text-slate-400 disabled:cursor-not-allowed";
  const commonInputClass = "h-9 bg-slate-800 border border-slate-700 text-white text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full px-2.5 disabled:bg-slate-700 disabled:text-slate-400 disabled:cursor-not-allowed";
  const commonDisplayClass = "h-9 bg-slate-800 border border-slate-700 text-white text-sm rounded-lg block w-full px-2.5 flex items-center";
            ...
            <div>
                ...
                {user.role === Role.CHAIRMAN ? (
                    <select value={selectedCollege} onChange={(e) => setSelectedCollege(e.target.value as College)} className={commonSelectClass}>
                        {Object.entries(COLLEGE_NAMES).map(([key, name]) => (<option key={key} value={key}>{name.split(' - ')[0]}</option>))}
                    </select>
                ) : (
                    <div className={commonDisplayClass}>{COLLEGE_NAMES[selectedCollege]}</div>
                )}
            </div>
            <div>
             ...
             <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">Admission Year</label>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
            ...
            <div className="lg:col-start-4 flex justify-end">
                <button
                    ...
                    disabled={isDownloading || filterConflictError}
                    className="w-full md:w-auto h-9 flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded-lg transition disabled:bg-slate-600 disabled:cursor-not-allowed"
                >
  ...
  return (
    <main className="flex-1 p-8 bg-transparent overflow-y-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        {(() => {
            if (filterConflictError || !dashboardData) {
                return null;
            }

            const isFacultyDrilldown = selectedFacultyId !== 'all';
            const isStaffDrilldown = selectedStaffId !== 'all';
            const isStudentRollNoDrilldown = selectedRollNo !== 'all';
            const isDepartmentView = selectedDepartment !== 'all';

            if (isFacultyDrilldown) {
                return <MetricSection title="Faculty Metrics" metrics={dashboardData.facultyMetrics} type="faculty" />;
            }
            if (isStaffDrilldown) {
                return <MetricSection title="Staff Metrics" metrics={dashboardData.staffMetrics} type="staff" />;
            }
            if (isStudentRollNoDrilldown) {
                 return <>
