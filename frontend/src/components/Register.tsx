...
import { Role, College } from '@shared/types';
...
const Register: React.FC<RegisterProps> = ({ onRegister, switchToLogin }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Validate username format for specific roles
    if (role === Role.FACULTY) {
        const facultyIdRegex = new RegExp(`^[KBG](${DEPARTMENTS.join('|')})\\d{8}-\\d{3}$`);
        if (!facultyIdRegex.test(username.toUpperCase())) {
            setError('For Faculty, username must be a valid Faculty ID (e.g., KCSE15042020-001).');
            return;
        }
    } else if (role === Role.STAFF) {
        const staffIdRegex = new RegExp(`^[KBG]STF\\d{8}-\\d{3}$`);
        if (!staffIdRegex.test(username.toUpperCase())) {
            setError('For Staff, username must be a valid Staff ID (e.g., KSTF15042020-001).');
            return;
        }
    }
      ...
      <div className="bg-white p-8 rounded-xl shadow-lg space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input type="text" value={username} onChange={e => setUsername(e.target.value)} placeholder="Enter username or required ID" className={inputClass('username')} onFocus={() => setFocused('username')} onBlur={() => setFocused(null)} />
          </div>
          <div>
            ...
            <select value={role} onChange={e => setRole(e.target.value as Role)} className={inputClass('role')} onFocus={() => setFocused('role')} onBlur={() => setFocused(null)}>
              {Object.entries(ROLE_DISPLAY_NAMES).map(([roleValue, roleName]) => <option key={roleValue} value={roleValue}>{roleName}</option>)}
            </select>
          </div>
          {role !== Role.CHAIRMAN && (
            <div>
