...
const getFacultyForCollegeAndDept = (college: College, department: string): Faculty[] => {
    let faculty = (college === College.ALL) ? mockFaculty : mockFaculty.filter(f => f.collegeCode === college);

    if (department !== 'all') {
        faculty = faculty.filter(f => f.programCode === department);
    }
    return faculty;
};

const getStaffForCollegeAndDept = (college: College, department: string): Staff[] => {
    let staff = (college === College.ALL) ? mockStaff : mockStaff.filter(s => s.collegeCode === college);

    if (department !== 'all') {
        staff = staff.filter(s => s.programCode === department);
    }
    return staff;
};
...
export const getDashboardData = async (
  ...
  const relevantFaculty = getFacultyForCollegeAndDept(college, department);
  ...
  const relevantStaff = getStaffForCollegeAndDept(college, department);
...
export const getFilteredStaffDetails = async (staffId: string, startDate: string, endDate: string): Promise<DetailedStaffCSVData[]> => {
    ...
    const result: DetailedStaffCSVData = {
        ...
        programCode: staffMember.programCode,
