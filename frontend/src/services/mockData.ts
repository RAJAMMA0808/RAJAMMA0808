import { Student, StudentMark, StudentAttendance, Faculty, FacultyAttendance, Staff, StaffAttendance } from '@shared/types';
import { LATEST_ATTENDANCE_DATE, DEPARTMENTS } from '../constants';
...
const staffJoinData = {
    KNRCER: [{ date: '15042020', count: 5 }],
    BIET: [{ date: '10072021', count: 2 }, { date: '11082022', count: 3 }], // Total 5
    BGIIT: [{ date: '05012022', count: 5 }],
};
...
Object.entries(staffJoinData).forEach(([collegeCode, joinGroups]) => {
    ...
    joinGroups.forEach(group => {
        for (let i = 1; i <= group.count; i++) {
            ...
            const programCode = departments[staffCounter % departments.length]; // Assign staff to depts cyclically
